...
async fn test_stability_multibyte_boundaries() {
    ...
    // We force a max_chunk_bytes of 2, which lands INSIDE the multibyte char (index 2).
    ...
    let params = ChunkParams {
        ...
        max_chunk_bytes: Some(2), // AGGRESSIVE limit
        chunk_overlap: 0,
    ...
    // Note: FixedWidthChunker might split "H" and "\u{FFFD}llo" separately.
    println!(
        "Multibyte chunks: {:?}",
...
async fn test_stability_binary_simulation() {
    ...
    // The TextSplitter might behave weirdly, but FixedWidthChunker must handle it fallback.
    ...
    let params = ChunkParams {
        target_chunk_size: 10,
        max_chunk_bytes: Some(5), // Force fallback frequently
        chunk_overlap: 0,
        tokenizer: "bytes".to_string(), // Char tokenizer handles nulls better than token types potentially
        file_extension: None,
    };
