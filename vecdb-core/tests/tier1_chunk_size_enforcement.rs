use vecdb_core::chunking::{Chunker, ChunkParams, RecursiveChunker};

#[tokio::test]
async fn test_recursive_chunker_enforces_max_size() {
    let chunker = RecursiveChunker;
    let _params = ChunkParams { // Unused check params
        chunk_size: 100,
    ...
    let chunks = chunker.chunk(&text, &bad_params).await.expect("Chunking failed");
    ...
    for (i, chunk) in chunks.iter().enumerate() {
        assert!(chunk.content.len() <= 100, "Chunk {} size {} exceeds max 100", i, chunk.content.len());
    }
...
async fn test_code_chunker_enforces_max_size() {
    use vecdb_core::chunking::{CodeChunker, Chunker};
    ...
    for (i, chunk) in chunks.iter().enumerate() {
        assert!(chunk.content.len() <= 50, "Chunk {} size {} exceeds max 50", i, chunk.content.len());
    }
}
