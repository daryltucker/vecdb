...
impl vecdb_core::backend::Backend for MockBackend {
    async fn upsert(&self, _collection: &str, chunks: Vec<vecdb_core::types::Chunk>) -> anyhow::Result<()> { 
    ...
    // Stubs
    async fn search(&self, _: &str, _: &[f32], _: u64, _: Option<serde_json::Value>) -> anyhow::Result<Vec<vecdb_core::types::SearchResult>> { Ok(vec![]) }
    async fn delete_collection(&self, _: &str) -> anyhow::Result<()> { Ok(()) }
    async fn collection_exists(&self, _: &str) -> anyhow::Result<bool> { Ok(true) }
    async fn create_collection(&self, _name: &str, _v: u64, _q: Option<vecdb_core::config::QuantizationType>) -> anyhow::Result<()> { Ok(()) }
    ...
    async fn get_collection_info(&self, _: &str) -> anyhow::Result<vecdb_core::types::CollectionInfo> { Ok(vecdb_core::types::CollectionInfo { name: "test".to_string(), vector_count: None, vector_size: None, quantization: None }) }
...
impl vecdb_core::embedder::Embedder for MockEmbedder {
    async fn embed(&self, _: &str) -> anyhow::Result<Vec<f32>> { Ok(vec![0.0; 384]) }
    async fn embed_batch(&self, texts: &[String]) -> anyhow::Result<Vec<Vec<f32>>> { Ok(vec![vec![0.0; 384]; texts.len()]) }
...
impl vecdb_core::parsers::ParserFactory for PassThroughFactory {
    ...
    fn get_streaming_parser(&self, file_type: FileType) -> Option<Box<dyn vecdb_core::parsers::Parser>> {
         match file_type {
...
async fn test_large_file_bifurcation_ast() {
    ...
    let result = vecdb_core::ingestion::ingest_path(&backend, &embedder, &detector, &factory, options).await;
...
async fn test_large_file_streaming_json() {
    ...
    f.write_all(b"{\"id\": 999, \"content\": \"last\"}]").unwrap();
