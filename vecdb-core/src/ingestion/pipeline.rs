    ...
    if !new_chunks.is_empty() {
        ...
        for chunk in new_chunks {
            if chunk.content.len() > MAX_CHUNK_CHARS {
                debug!("Warning: Oversized chunk detected ({} chars). Splitting...", chunk.content.len());
                let sub_chunks: Vec<crate::chunking::ChunkResult> = fallback_chunker.chunk(&chunk.content, &fallback_params).await?;
