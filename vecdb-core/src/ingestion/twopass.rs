...
impl TwoPassIngestor {
    pub async fn process_large_file(
        ...
        while offset < file_size {
            let read_size = std::cmp::min(segment_size + overlap_size, (file_size - offset) as usize);
            ...
            let segment_chunks = if let Some(p) = parser_factory.get_parser(file_type) {
                ...
                c.push(Chunk {
                    ...
                    vector: None,
                    metadata: metadata.clone().unwrap_or(serde_json::json!({})).as_object().unwrap().clone().into_iter().collect(),
                    page_num: None,
