...
impl Backend for QdrantBackend {
    ...
    async fn upsert(&self, collection: &str, chunks: Vec<Chunk>) -> Result<()> {
        ...
        let points: Vec<PointStruct> = chunks
            ...
            .map(|chunk| {
                    ...
                    match v {
                        ...
                        serde_json::Value::Number(n) => {
                            if let Some(i) = n.as_i64() {
                                ...
                                Value { kind: Some(Kind::DoubleValue(n.as_f64().unwrap_or(0.0))) }
                            }
                        },
                        serde_json::Value::String(s) => Value { kind: Some(Kind::StringValue(s)) },
                        serde_json::Value::Array(arr) => {
                            let values = arr.into_iter().map(json_to_qdrant).collect();
                            Value { kind: Some(Kind::ListValue(qdrant_client::qdrant::ListValue { values })) }
                        },
                        serde_json::Value::Object(_) => Value { kind: Some(Kind::StringValue("Nested objects not supported yet".into())) }, // Simplification
                    }
                }
                ...
                payload.insert("content".to_string(), Value { kind: Some(Kind::StringValue(chunk.content)) });
                payload.insert("document_id".to_string(), Value { kind: Some(Kind::StringValue(chunk.document_id)) });
    ...
    async fn search(
        ...
        let results = search_result.result.into_iter().map(|scored_point| {
            ...
            fn qdrant_to_json(v: Value) -> serde_json::Value {
                match v.kind {
                    ...
                    Some(Kind::ListValue(l)) => serde_json::Value::Array(l.values.into_iter().map(qdrant_to_json).collect()),
                    Some(Kind::StructValue(s)) => {
                        let map = s.fields.into_iter().map(|(k, v)| (k, qdrant_to_json(v))).collect();
            ...
            let id_str = match scored_point.id {
                Some(PointId { point_id_options: Some(PointIdOptions::Uuid(u)) }) => u,
    ...
    async fn list_metadata_values(&self, collection: &str, key: &str) -> Result<Vec<String>> {
        ...
        loop {
            // Remove & reference
            let result = self.client.scroll(ScrollPoints {
                collection_name: collection.to_string(),
            ...
            for point in result.result {
                if let Some(val) = point.payload.get(key) {
                    match &val.kind {
                        Some(Kind::StringValue(s)) => { values.insert(s.clone()); },
                        Some(Kind::IntegerValue(i)) => { values.insert(i.to_string()); },
