...
impl SnapshotManager {
    ...
    pub async fn list(&self, collection: &str) -> Result<Vec<String>> {
        let url = self
            .base_url
            .join(&format!("collections/{}/snapshots", collection))?;
    ...
    pub async fn restore(&self, collection: &str, file_path: &Path) -> Result<()> {
        ...
        let url = self
            .base_url
            .join(&format!("collections/{}/snapshots/upload", collection))?;
        ...
        if !resp.status().is_success() {
            ...
            return Err(anyhow::anyhow!(
                ...
                text,
                "Check if collection exists or try creating it first."
            ));
        }
