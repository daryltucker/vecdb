        ...
        impl<'de> Visitor<'de> for ChunkVisitor {
                ...
                A: SeqAccess<'de>,
            {
                ...
                while let Some(value) = seq.next_element::<Value>()? {
                    ...
                    let mut metadata: std::collections::HashMap<String, Value> = match &self.base_metadata {
                    ...
                    metadata.insert("stream_index".to_string(), serde_json::json!(item_count));
                     ...
                     if let Some(ts) = value.get("timestamp").or(value.get("time")).or(value.get("created_at")) {
                         if let Some(ts_str) = ts.as_str() {
                             metadata.insert("timestamp".to_string(), Value::String(ts_str.to_string()));
                         }
                    }
...
impl Parser for StreamingJsonParser {
    async fn parse(&self, _content: &str, path: &Path, base_metadata: Option<Value>) -> Result<Vec<Chunk>> {
        let path_buf = path.to_path_buf();
        let meta = base_metadata.clone();
        let myself = Self {}; 
        
        let chunks = tokio::task::spawn_blocking(move || {
            myself.stream_file(&path_buf, &meta)
        }).await??;
