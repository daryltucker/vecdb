...
impl VecqParserAdapter {
    ...
    fn flatten_elements(&self, elements: &[DocumentElement], chunks: &mut Vec<Chunk>, path: &Path, base_metadata: &serde_json::Value, parent_trail: &str, doc_id: &str) {
        for element in elements {
             // Basic metadata
             let mut metadata = base_metadata.as_object().cloned().unwrap_or_default();
             metadata.insert("element_type".to_string(), serde_json::Value::String(element.element_type.to_string()));
             if let Some(name) = &element.name {
                 metadata.insert("name".to_string(), serde_json::Value::String(name.clone()));
             }
             metadata.insert("line_start".to_string(), serde_json::json!(element.line_start));
             ...
             metadata.insert("file_type".to_string(), serde_json::Value::String(self.file_type.to_string()));
             ...
             let current_trail = if parent_trail.is_empty() {
                 element.name.clone().unwrap_or(element.element_type.to_string())
             } else {
                 format!("{}::{}", parent_trail, element.name.as_deref().unwrap_or(&element.element_type.to_string()))
             };
             ...
             if !element.children.is_empty() {
                 self.flatten_elements(&element.children, chunks, path, base_metadata, &current_trail, doc_id);
             }
        }
    }
}
...
impl Parser for VecqParserAdapter {
    async fn parse(&self, content: &str, path: &Path, base_metadata: Option<serde_json::Value>) -> Result<Vec<Chunk>> {
        ...
        self.flatten_elements(&parsed_doc.elements, &mut chunks, path, &base_meta, "", &doc_id);
