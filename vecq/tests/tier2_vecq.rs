...
fn test_grep_format_parity() {
    ...
    // 3. Verify Execution
    assert!(output.status.success(), "vecq failed: {}", String::from_utf8_lossy(&output.stderr));
    ...
    // 1. "unknown" filename
    assert!(!stdout.contains("unknown:"), "Found 'unknown' filename in output: {}", stdout);
    
    // 2. "null" noise (Regress: noise suppression)
    // grep format shouldn't output ":null" or lines with just separators if no content
    assert!(!stdout.contains(":null"), "Found explicit null in output: {}", stdout);
