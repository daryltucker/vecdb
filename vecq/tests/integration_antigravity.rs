...
use vecq::types::{ElementType, FileType};
...
"#;
    ...
    let system = doc
        .elements
        .iter()
        .find(|e| e.name.as_deref() == Some("system"))
        .expect("system tag not found");
    // XML parser streaming: children are in e.children
    let role = system
        .children
        .iter()
        .find(|e| e.name.as_deref() == Some("role"))
        .expect("role tag not found");
    assert_eq!(role.content, "Assistant");

    let task = doc
        .elements
        .iter()
        .find(|e| e.name.as_deref() == Some("task"))
        .expect("task tag not found");
    ...
    let header = task
        .children
        .iter()
        .find(|e| e.element_type == ElementType::Header);

    assert!(
        header.is_some(),
        "Markdown Header not found in task children"
    );
