...
fn test_all_parsers_have_fixtures() {
    ...
    for parser_type in parsers {
        ...
        if !parser_dir.exists() || !parser_dir.is_dir() {
            ...
            panic!(
                "MISSING FIXTURES: No directory found for {:?} at {:?}",
                parser_type, parser_dir
            );
        }
        ...
        if entries.is_empty() {
            panic!(
                "EMPTY FIXTURES: No test files found for {:?} in {:?}",
                parser_type, parser_dir
            );
        }
        ...
        for entry in entries {
            ...
            match result {
                Ok(doc) => {
                    // Sanity assertions
                    assert_eq!(
                        doc.metadata.file_type, parser_type,
                        "File type mismatch in metadata for {:?}",
                        path
                    );
                ...
                Err(e) => {
                    panic!(
                        "PARSING FAILED: {:?} failed to parse fixture {:?}: {}",
                        parser_type, path, e
                    );
                }
            }
