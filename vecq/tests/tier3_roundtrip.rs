...
use vecq::generators::{JsonGenerator, RustGenerator, TomlGenerator};
use vecq::parser::Parser;
#[cfg(feature = "javascript-parser")]
use vecq::parsers::JavaScriptParser;
use vecq::parsers::{
    BashParser, CParser, CppParser, CudaParser, GoParser, JsonParser, PythonParser, RustParser,
    TomlParser,
};
...
async fn verify_round_trip(
    ...
    if data_mode {
        ...
        let doc2 = parser
            .parse(&generated)
            .await
            .expect("Failed to re-parse generated code");

        // Naive verification: Count elements
        assert_eq!(
            doc.elements.len(),
            doc2.elements.len(),
            "Element count mismatch in {}",
            name
        );
        ...
        if !doc.elements.is_empty() {
            ...
            let c1 = doc.elements[0]
                .content
                .split_whitespace()
                .collect::<Vec<_>>()
                .join(" ");
            let c2 = doc2.elements[0]
                .content
                .split_whitespace()
                .collect::<Vec<_>>()
                .join(" ");
...
"#;
    ...
    let pkg_el = doc
        .elements
        .iter()
        .find(|e| e.name.as_deref() == Some("package"))
        .unwrap();
    if let ElementAttributes::Toml(_) = &pkg_el.attributes {
        println!(
            "Verified strict Toml attributes for '{}'",
            pkg_el.name.as_ref().unwrap()
        );
    } else {
        panic!(
            "Expected ElementAttributes::Toml, found {:?}",
            pkg_el.attributes
        );
    }
...
"#;
    ...
    let fn_el = doc
        .elements
        .iter()
        .find(|e| e.name.as_deref() == Some("add"))
        .unwrap();
    ...
    let vis = fn_el
        .attributes
        .get_text("visibility")
        .expect("Missing visibility attribute");
    println!(
        "Verified attributes for '{}': vis={}",
        fn_el.name.as_ref().unwrap(),
        vis
    );
    assert_eq!(vis, "pub");
    ...
    #[cfg(feature = "javascript-parser")]
    {
        ...
        let fn_el = doc
            .elements
            .iter()
            .find(|e| e.name.as_deref() == Some("hello"))
            .unwrap();
        if let ElementAttributes::JavaScript(attr) = &fn_el.attributes {
            println!(
                "Verified strict JS attributes for '{}': async={}",
                fn_el.name.as_ref().unwrap(),
                attr.is_async
            );
            ...
            panic!(
                "Expected ElementAttributes::JavaScript, found {:?}",
                fn_el.attributes
            );
        }
    }
  ...
  "dependencies": {
    ...
    if let ElementAttributes::Json(_) = &name_el.attributes {
        println!(
            "Verified strict JSON attributes for '{}'",
            name_el.name.as_ref().unwrap()
        );
    } else {
        panic!(
            "Expected ElementAttributes::Json, found {:?}",
            name_el.attributes
        );
    }
    ...
    if let ElementAttributes::Python(attr) = &fn_el.attributes {
        println!(
            "Verified strict Python attributes for '{}': async={}",
            fn_el.name.as_ref().unwrap(),
            attr.is_async
        );
        assert!(attr.is_async);
        ...
        panic!(
            "Expected ElementAttributes::Python, found {:?}",
            fn_el.attributes
        );
    }
    ...
    let kernel_el = doc
        .elements
        .iter()
        .find(|e| e.name.as_deref() == Some("my_kernel"))
        .unwrap();
    if let ElementAttributes::CFamily(attr) = &kernel_el.attributes {
        let is_global = attr
            ...
            .unwrap_or(false);
        println!(
            "Verified strict CFamily (CUDA) attributes for '{}': is_global={}",
            kernel_el.name.as_ref().unwrap(),
            is_global
        );
        assert!(is_global);
    } else {
        panic!(
            "Expected ElementAttributes::CFamily, found {:?}",
            kernel_el.attributes
        );
    }
    ...
    let c_el = doc
        .elements
        .iter()
        .find(|e| e.name.as_deref() == Some("my_c_func"))
        .unwrap();
    ...
    let cpp_el = doc
        .elements
        .iter()
        .find(|e| e.name.as_deref() == Some("MyClass"))
        .unwrap();
    ...
    if let ElementAttributes::Go(_) = &fn_el.attributes {
        println!(
            "Verified strict Go attributes for '{}'",
            fn_el.name.as_ref().unwrap()
        );
    } else {
        panic!(
            "Expected ElementAttributes::Go, found {:?}",
            fn_el.attributes
        );
    }
    ...
    if let ElementAttributes::Bash(_) = &fn_el.attributes {
        println!(
            "Verified strict Bash attributes for '{}'",
            fn_el.name.as_ref().unwrap()
        );
    } else {
        panic!(
            "Expected ElementAttributes::Bash, found {:?}",
            fn_el.attributes
        );
    }
