...
async fn test_basic_query_rust() {
    ...
    let results = query_json(&json, ".functions[] | select(.attributes.visibility == \"pub\")").unwrap();
    ...
    assert_eq!(results[0].get("name").and_then(|v| v.as_str()), Some("main"));
...
class Cat:
    ...
    assert_eq!(results.len(), 1);
    assert_eq!(results[0], serde_json::Value::Number(serde_json::Number::from(1)));
