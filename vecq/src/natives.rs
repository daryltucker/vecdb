...
use jaq_core::Exn;
use jaq_core::{Bind, Ctx, Error, Native};
use jaq_json::Val;
use regex::Regex;
use std::boxed::Box;
use std::rc::Rc;
...
fn test_impl(input_val: Val, regex_val: Val) -> Box<dyn Iterator<Item = Result<Val, Exn<Val>>>> {
    ...
    let re = match Regex::new(&regex_str) {
        Ok(r) => r,
        Err(e) => {
            return Box::new(std::iter::once(Err(Exn::from(Error::new(Val::from(
                e.to_string(),
            ))))))
        }
    };
...
fn match_impl(
    ...
    regex_val: Val,
    global: bool,
) -> Box<dyn Iterator<Item = Result<Val, Exn<Val>>>> {
    let input = match get_string(&input_val) {
        Ok(s) => s,
        Err(e) => return Box::new(std::iter::once(Err(Exn::from(e)))),
    };
    ...
    let matches: Vec<Val> = re
        ...
        .map(|caps| {
            ...
            obj.insert(
                Val::from("length".to_string()),
                Val::from(m.end() - m.start()),
            );
            obj.insert(
                Val::from("string".to_string()),
                Val::from(m.as_str().to_string()),
            );
            ...
            for i in 1..caps.len() {
                if let Some(c) = caps.get(i) {
                    ...
                    cap_obj.insert(
                        Val::from("length".to_string()),
                        Val::from(c.end() - c.start()),
                    );
                    cap_obj.insert(
                        Val::from("string".to_string()),
                        Val::from(c.as_str().to_string()),
                    );
            ...
            obj.insert(
                Val::from("captures".to_string()),
                Val::Arr(Rc::new(captures)),
            );
            Val::obj(obj)
        })
        .collect();
...
pub fn regex_natives() -> impl Iterator<
    Item = (
        &'static str,
        Box<[Bind]>,
        Native<jaq_core::data::JustLut<Val>>,
    ),
> {
    vec![
        (
            "_native_test",
            Box::new([]) as Box<[Bind]>,
            Native::new(|(_, val): (Ctx<jaq_core::data::JustLut<Val>>, Val)| {
                let arr = match val {
                    Val::Arr(ref a) => a,
                    _ => {
                        return Box::new(std::iter::once(Err(Exn::from(Error::new(Val::from(
                ...
                if arr.len() < 2 {
                    return Box::new(std::iter::once(Err(Exn::from(Error::new(Val::from(
                        "Expected 2 args for test".to_string(),
                    ))))));
                }
        ...
        (
            ...
            Native::new(|(_, val): (Ctx<jaq_core::data::JustLut<Val>>, Val)| {
                ...
                if arr.len() < 2 {
                    return Box::new(std::iter::once(Err(Exn::from(Error::new(Val::from(
                        "Expected 2 args for capture".to_string(),
                    ))))));
                }
        ...
        (
            ...
            Native::new(|(_, val): (Ctx<jaq_core::data::JustLut<Val>>, Val)| {
                ...
                if arr.len() < 2 {
                    return Box::new(std::iter::once(Err(Exn::from(Error::new(Val::from(
                        "Expected 2 args for match".to_string(),
                    ))))));
                }
        ...
        (
            ...
            Native::new(|(_, val): (Ctx<jaq_core::data::JustLut<Val>>, Val)| {
                ...
                if arr.len() < 2 {
                    return Box::new(std::iter::once(Err(Exn::from(Error::new(Val::from(
                        "Expected 2 args for splits".to_string(),
                    ))))));
                }
        ...
        (
            ...
            Native::new(|(_, val): (Ctx<jaq_core::data::JustLut<Val>>, Val)| {
                ...
                if arr.len() < 3 {
                    return Box::new(std::iter::once(Err(Exn::from(Error::new(Val::from(
                        "Expected 3 args for sub".to_string(),
                    ))))));
                }
        ...
        (
            ...
            Native::new(|(_, val): (Ctx<jaq_core::data::JustLut<Val>>, Val)| {
                ...
                if arr.len() < 3 {
                    return Box::new(std::iter::once(Err(Exn::from(Error::new(Val::from(
                        "Expected 3 args for gsub".to_string(),
                    ))))));
                }
