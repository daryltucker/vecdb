...
use crate::types::{
    CFamilyAttributes, DocumentElement, DocumentMetadata, ElementAttributes, ElementType, FileType,
    ParsedDocument,
};
        ...
        for child in node.children(&mut cursor) {
            ...
            match kind {
                "namespace_definition" => {
                    let mut element =
                        self.create_element(child, content, source, ElementType::Namespace);
                ...
                "class_specifier" | "struct_specifier" => {
                    let element_type = if kind == "class_specifier" {
                        ElementType::Class
                    } else {
                        ElementType::Struct
                    };
                ...
                if child.kind() == "function_declarator" {
                    ...
                    for deep_child in child.children(&mut d) {
                        if deep_child.kind() == "identifier"
                            || deep_child.kind() == "field_identifier"
                            || deep_child.kind() == "qualified_identifier"
                        {
                            name = deep_child
                                .utf8_text(content.as_bytes())
                                .unwrap_or("")
...
"#;
        ...
        let has_foo = ns
            .children
            .iter()
            .any(|c| c.name.as_deref() == Some("foo") && c.element_type == ElementType::Function);
        let has_internal = ns
            .children
            .iter()
            .any(|c| c.name.as_deref() == Some("Internal") && c.element_type == ElementType::Class);
