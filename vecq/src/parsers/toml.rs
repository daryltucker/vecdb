...
impl TomlParser {
        ...
        match item {
            toml_edit::Item::Value(v) => match v {
                ...
                toml_edit::Value::Array(arr) => serde_json::Value::Array(
                    arr.iter()
                        .map(|v| Self::convert_value(&toml_edit::Item::Value(v.clone())))
                        .collect(),
                ),
                toml_edit::Value::InlineTable(table) => {
                    ...
                    for (k, v) in table.iter() {
                        map.insert(
                            k.to_string(),
                            Self::convert_value(&toml_edit::Item::Value(v.clone())),
                        );
                    }
            ...
            toml_edit::Item::ArrayOfTables(arr) => serde_json::Value::Array(
                arr.iter()
                    .map(|t| Self::convert_value(&toml_edit::Item::Table(t.clone())))
                    .collect(),
            ),
    ...
    fn process_item(
        ...
        let end_line = counter
            .get_line_number(span.end.saturating_sub(1))
            .max(start_line);

        // Content is `key = value`, not the bare value.
        //
        // This string is what gets embedded, and `item.to_string()` for a leaf
        // is just the value — so `vecdb-core/Cargo.toml` produced four separate
        // chunks whose entire content was ` true`. They embed to the same
        // vector, carry no indication of which key they belong to, and are
        // indistinguishable in a search result. The key lives in `name` and the
        // path in `crumbtrail`, but those are payload metadata: filterable, not
        // matchable.
        //
        // Reconstructing `key = value` matches the source line and keeps TOML
        // and YAML consistent — a query that works on one should work on the
        // other, since they describe the same kind of thing.
        let content = match item {
            toml_edit::Item::Table(_) | toml_edit::Item::ArrayOfTables(_) => String::new(),
            leaf => format!("{key} ={}", leaf.to_string().trim_end()),
        };
        ...
        match item {
            ...
            toml_edit::Item::ArrayOfTables(arr) => {
                ...
                for (idx, table) in arr.iter().enumerate() {
                    ...
                    element
                        .children
                        .push(self.process_item(&key_with_idx, &table_item, counter));
                }
            }
            _ => {
                // Leaf value
        ...
        element = element
            .set_attributes(ElementAttributes::Toml(TomlAttributes {
                other: HashMap::new(),
            }))
