...
/// Attributes for usage/reference elements
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub struct UsageAttributes {
    pub symbol_name: String, // What symbol is being used
    pub usage_type: String,  // Type of usage (call, reference, assignment, etc.)
    pub context: String,     // Surrounding context (function name, etc.)
    pub scope: String,       // Lexical scope where usage occurs
    #[serde(flatten)]
    pub other: HashMap<String, serde_json::Value>,
}
...
impl ElementAttributes {
    ...
    pub fn get_text(&self, key: &str) -> Option<String> {
        match self {
            ...
            Self::Rust(attr) => {
                if key == "visibility" {
                    return Some(attr.visibility.clone());
                }
                attr.other
                    .get(key)
                    .and_then(|v| v.as_str())
            ...
            Self::JavaScript(attr) => {
                if key == "is_async" {
                    return Some(attr.is_async.to_string());
                }
                if key == "is_arrow" {
                    return Some(attr.is_arrow.to_string());
                }
                attr.other
                    .get(key)
                    .and_then(|v| v.as_str())
            ...
            Self::Usage(attr) => {
                if key == "symbol_name" {
                    return Some(attr.symbol_name.clone());
                }
                if key == "usage_type" {
                    return Some(attr.usage_type.clone());
                }
                if key == "context" {
                    return Some(attr.context.clone());
                }
                if key == "scope" {
                    return Some(attr.scope.clone());
                }
                attr.other
                    .get(key)
                    .and_then(|v| v.as_str())
    ...
    pub fn is_empty(&self) -> bool {
        match self {
            ...
            Self::Usage(attr) => {
                attr.symbol_name.is_empty()
                    && attr.usage_type.is_empty()
                    && attr.context.is_empty()
                    && attr.scope.is_empty()
                    && attr.other.is_empty()
            }
        }
    ...
    pub fn get(&self, key: &str) -> Option<&serde_json::Value> {
        match self {
            ...
            Self::Usage(attr) => attr.other.get(key),
        }
    }
