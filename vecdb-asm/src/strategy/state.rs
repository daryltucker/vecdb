...
use crate::types::{Snapshot, Timeline, EvolutionEvent, TimelineAnalysis, BranchReason};
    ...
    for record in records {
        ...
        if let Some(caps) = re.captures(path_str) {
            ...
            if let Ok(version) = version_str.parse::<usize>() {
                ...
                artifacts.entry(base_path).or_default().insert(version, snapshot);
            }
        }
    }
...
mod tests {
    ...
    fn test_massive_rewrite_detection() {
        ...
        match &branch.reason {
            BranchReason::MassiveRewrite { deletion_ratio, overlap_ratio } => {
